...
const SideBar: React.FC<SideBarProps> = (_props): JSX.Element => {
  ...
  return (
    // <div className='flex flex-col items-center w-full h-full bg-[#101923] pt-5'>
    //   {
    //     sidebarRoutes.section3.map(({ href, name, id }) => {
    //       const isActive = href.split('/')[1] === router.pathname.split('/')[1]
    //       return (
    //         <Link href={href} passHref={true} key={id}>
    //           <div className={`p-1.5 my-1 duration-200 ease-out w-full cursor-pointer group hover:bg-slate-600 ${isActive ? 'bg-slate-600' : 'bg-transparent'} rounded-md w-5/6`}
    //           >
    //             <label className='w-full ml-2 duration-200 ease-out cursor-pointer group-hover:text-white'>{name}</label>
    //           </div>
    //         </Link>
    //       )
    //     })

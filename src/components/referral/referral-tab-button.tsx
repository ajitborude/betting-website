...
const ReferralTabButton: React.FC<Props> = ({ text, selected, type }) => {
  let classes = {
    left: '',
    center: '',
    right: '',
  ...
  switch (type) {
    case 'start':
      if (selected) classes = { left: 'tab-odd-left', center: 'tab-odd-center', right: 'tab-odd-right', extra: 'tab-start-referral' };
      else classes = { left: 'tab-odd-left-inc', center: 'tab-odd-center-inc', right: 'tab-odd-right-inc', extra: 'tab-start-referral' };
      break;
    case 'even':
    ...
    case 'odd':
      ...
      break;
    case 'end-even':
      if (selected) classes = { left: 'tab-even-left', center: 'tab-even-center', right: 'tab-even-right', extra: 'tab-end-even-referral' };
      else classes = { left: 'tab-even-left-inc', center: 'tab-even-center-inc', right: 'tab-even-right-inc', extra: 'tab-end-even-referral' };
      break;
    case 'end-odd':
  ...
  return (
    <div className={`flex w-full relative !h-16 group cursor-pointe outline-0 ${classes.extra}`}>
      <div className={`h-full w-[25%] flex justify-center items-center ${classes.left}`} />
      <div className={`h-full w-[52%] flex justify-center items-center  ${classes.center}`} />
      <div className={`h-full w-[25%] flex justify-center items-center ${classes.right}`} />
...
export default ReferralTabButton;
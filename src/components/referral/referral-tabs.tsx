...
import ReferralTabButton from '../referral/referral-tab-button';

interface Props {
  defaultTab: number;
}

const ReferralTabs: React.FC<Props> = ({ defaultTab = 1 }): JSX.Element => {
  ...
  return (
    <div className="w-full pt-16">
      <Tab.Group selectedIndex={defaultTab} onChange={(index) => router.push(`/referral?tab=${index}`)}>
            ...
            {({ selected }) => (
              ...
              >
                <div className={`left-[0] w-[29%] flex items-center justify-center rounded-none absolute cursor-pointer`}>
                  <ReferralTabButton text={tabs[0]?.title} type='start' selected={selected} />
                </div>
              </Transition >
            )}
          </Tab>
            ...
            {({ selected }) => (
              ...
              >
                <div className={`left-[24%] w-[29%] flex items-center justify-center rounded-none absolute cursor-pointer`}>
                  <ReferralTabButton text={tabs[1]?.title} type='even' selected={selected} />
                </div>
              </Transition >
            )}
          </Tab>
            ...
            {({ selected }) => (
              ...
              >
                <div className={`left-[48%] w-[29%] flex items-center justify-center rounded-none absolute cursor-pointer`}>
                  <ReferralTabButton text={tabs[2]?.title} type='odd' selected={selected} />
                </div>
              </Transition >
            )}
          </Tab>
            ...
            {({ selected }) => (
              ...
              >
                <div className={`left-[72%] w-[29%] flex items-center justify-center rounded-none absolute cursor-pointer`}>
                  <ReferralTabButton text={tabs[3]?.title} type='end-even' selected={selected} />
                </div>
              </Transition >
            )}
          </Tab>

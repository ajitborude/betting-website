import React, { useState, useRef, useReducer, Fragment, useEffect } from "react";
import Frame from "@/public/login/frame.png";
import SimpleReactValidator from "simple-react-validator";
import { login, register, metamaskLogin, metamaskVerify, googleLogin } from '@/redux/slices/auth';
...
const TextInput = dynamic(() => import('@/components/common/text-input'), { loading: () => <div /> });
const PrimaryButton = dynamic(() => import('@/components/common/primary-button'), { loading: () => <div /> });
const SecondaryButton = dynamic(() => import('@/components/common/secondary-button'), { loading: () => <div /> });
...
const LoginPage: NextPage<PageProps> = (_props) => {
  ...
  const handleGoogleLogin = useGoogleLogin({
    onSuccess: async codeResponse => {
      const resp: any = await dispatch(googleLogin({ code: codeResponse.code }))
      if (resp.error) {
  ...
  return (
    <div className='relative flex items-center justify-center w-full h-screen'>
      ...
      <div className='flex items-center justify-center w-[650px] max-w-full overflow-hidden scroll-smooth' >
        ...
        >
          <form className='relative flex flex-col items-center justify-between w-full h-full py-8' onSubmit={(e) => {
            e.preventDefault();
            handleAuth();
          }}>
            ...
            <div className=" w-[75%] h-full flex items-center justify-between flex-col -ml-3 ">
              ...
              >
                ...
                <div className='flex flex-row items-start justify-start w-full mt-4 mb-2' >
                  ...
                  <label className='ml-1 text-sm font-bold text-left'>
                    ...
                    {" "} and I have read the {" "}
                    <label className='w-full mb-2 text-sm font-bold underline cursor-pointer underline-offset-2 text-special-500'
              ...
              <label className='ml-12 text-sm font-bold text-left w-full text-error-500'> {showTncError ? 'Please accept terms of service' : ''}</label>
              <Transition
                show={isShowing}
                as={Fragment}
                appear={true}
              ...
              >
                <div className='flex w-full mt-2 mb-4'>
                  <PrimaryButton onClick={() => { }} btnType='submit'>
                    <label className={`text-center ${isLoginPage ? 'login-btn-label' : 'create-account-btn-label'}`}>
              ...
              >
                <div className='flex flex-col items-center justify-center w-[110%] login-v2-strip'>
                  <label className='w-full py-3 font-semibold text-center'>{isLoginPage ? "Don't have an account? " : "Already have an account? "}
                    {isLoginPage ?
                      <label className='w-full mb-2 text-sm font-bold underline uppercase cursor-pointer underline-offset-2 text-special-500' onClick={() => router.push('/login?isLogin=false')}>Sign Up Here</label>
                      :
                      <label className='w-full mb-2 text-sm font-bold underline uppercase cursor-pointer underline-offset-2 text-special-500' onClick={() => router.push('/login?isLogin=true')}>Login Here</label>
                    }
                  </label>
                </div>
              </Transition>
              ...
              >
                <div className='flex flex-col items-center justify-center w-full my-2 text-sm'>
                  <label className="text-lg font-semibold">{isLoginPage ? 'Or Login With' : 'Or Sign Up With'}</label>
                  <div className='flex flex-row items-center justify-around w-full my-4'>
                    <div className='flex w-[49%]'>
                      <SecondaryButton icon='google' size='md' onClick={() => {
                        if (!isAccepted && !isLoginPage) {
                          return setShowTncError(true)
                        }
                        handleGoogleLogin()
                    ...
                    <div className='flex w-[49%]'>
                      <SecondaryButton size='md' onClick={handleMetamask} icon='metamask' >
                        <label className='text-left metamask-btn-label'>metamask</label>
                      </SecondaryButton >
                    </div>
                  </div>
                </div>
